...
| `0x12` | `CMD_LONG_PRESS` | C → S | 6B | 长按 (x, y, duration) (new) |
| `0x13` | `CMD_MULTI_TOUCH` | C → S | Variable | 多点触控 (new) |
| `0x14` | `CMD_GESTURE` | C → S | Variable | 复杂手势 (捏合/旋转) (new) |
...
| `0x33` | `CMD_GET_FOCUSED` | C → S | None | 获取焦点元素 (new) |
| `0x34` | `CMD_WAIT_FOR` | C → S | Variable | 等待元素出现 (new) |
| `0x35` | `CMD_OCR_REGION` | C → S | 8B | 设备端 OCR (new) |

### Lifecycle Layer (0x40-0x4F) - 应用管理

| CMD ID | Name | Direction | Payload | Description |
|:-------|:-----|:----------|:--------|:------------|
| `0x40` | `CMD_RESTART_APP` | C → S | Variable | 重启应用 (new) |
| `0x41` | `CMD_GET_CLIPBOARD` | C → S | None | 获取剪贴板内容 (new) |
| `0x42` | `CMD_SET_CLIPBOARD` | C → S | Variable | 设置剪贴板内容 (new) |
...
| CMD ID | Name | Direction | Payload | Description |
|:-------|:-----|:----------|:--------|:------------|
| `0x60` | `CMD_SCREENSHOT` | C → S | None | 截图 (单帧，迁移自 0x09) |
